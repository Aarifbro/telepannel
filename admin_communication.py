...
def register_admin_communication_handlers(bot):
    ...
    def show_user_selection(call):
        """Show user selection interface for admin"""
        ...
        try:
            ...
            # Create user selection interface with pagination
            text = "👥 <b>Select User to Chat With</b>\n\n"
            text += "Choose a user from the list below:\n\n"
            ...
            if len(users) > 10:
                markup.add(types.InlineKeyboardButton(
                    f"📄 Show More Users ({len(users) - 10} remaining)",
                    callback_data="show_more_users_10"
                ))
            ...
            print(f"Error showing user selection: {e}")
    ...
    def start_chat_with_user(call):
        ...
        try:
            ...
            username = user_info[0] if user_info and user_info[0] else f"User {target_user_id}"
            
            text = (
                f"💬 <b>Admin Chat Session Started</b>\n\n"
                f"<b>Chatting with:</b> {username} (ID: {target_user_id})\n"
